...
async def admin_dashboard(message: Message):
    ...
    async with aiosqlite.connect(settings.db_name) as db:
        c_users = await db.execute("SELECT COUNT(*) FROM users")
        total_users = (await c_users.fetchone())[0]
        c_bal = await db.execute("SELECT SUM(balance), SUM(usdt_balance) FROM users")
        bal_row = await c_bal.fetchone()
        ton_sum, usdt_sum = bal_row[0] or 0.0, bal_row[1] or 0.0
        c_loans = await db.execute("SELECT SUM(loan_amount) FROM users WHERE loan_amount > 0")
        total_loans = (await c_loans.fetchone())[0] or 0.0
        
    freeze_btn_text = "🟢 توقف اضطراری: غیرفعال" if not settings.emergency_freeze else "🔴 توقف اضطراری: فعال (فریز است)"
    text = (
        f"👨‍💻 **مرکز مانیتورینگ زنده صرافی CX**\n\n"
        f"👥 کل تریدرها: `{total_users:,}`\n"
        f"💼 ذخایر TON صرافی: `{ton_sum:,.2f} TON`\n"
        f"💵 ذخایر USDT صرافی: `{usdt_sum:,.2f} USDT`\n"
        f"💸 مطالبات وام در جریان: `{total_loans:,.2f} TON`"
    )
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=freeze_btn_text, callback_data="toggle_emergency_freeze")],
        [InlineKeyboardButton(text="💰 افزایش بالانس تستی", callback_data="admin_add_bal")]
    ])
    await message.answer(text, reply_markup=kb, parse_mode="Markdown")

@router.callback_query(F.data == "toggle_emergency_freeze", StateFilter("*"))
async def cb_toggle_freeze(callback: CallbackQuery):
    if callback.from_user.id != settings.admin_id: return
    settings.emergency_freeze = not settings.emergency_freeze
    await callback.answer(f"وضعیت فریز اضطراری: {settings.emergency_freeze}", show_alert=True)
    await callback.message.delete()
    await admin_dashboard(callback.message)

@router.callback_query(F.data == "admin_add_bal", StateFilter("*"))
async def cb_admin_add_bal(callback: CallbackQuery, state: FSMContext):
    if callback.from_user.id != settings.admin_id: return
    await callback.message.answer("✍️ مبلغ مورد نظر برای افزایش را وارد کنید:")
...
async def process_admin_add_bal(message: Message, state: FSMContext):
    ...
    try:
        ...
        if len(parts) >= 2:
            ...
            amt = float(parts[0])
        if amt <= 0:
            return await message.answer("❌ مبلغ نامعتبر")
        ...
        await message.answer(f"❌ خطا: `{e}`", parse_mode="Markdown")
